...
def longest_word(letters):
    out_words = []
    for word in words:
        if all(x in letters for x in set(word)) and all(word.count(x) <= letters.count(x) for x in set(letters)):
            out_words.append(word)
    return [x for x in out_words if len(x) == len(max(out_words, key=len))] or None

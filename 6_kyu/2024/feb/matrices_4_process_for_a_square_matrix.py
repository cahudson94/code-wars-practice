...
Create a function avg_diags()that may receive a square matrix of uncertain dimensions and may output both averages in an array in the following order:
[avg1, avg2]
...
def avg_diags(m):
    ...
    for i in range(len(m)):
        if m[i][i] > -1 and i % 2:
            avg_1.append(m[i][i])
        if m[-i - 1][i] < 0 and not i % 2:
            avg_2.append(m[-i - 1][i])
    return [round(sum(avg_1) / len(avg_1)) if len(avg_1) else -1, round(abs(sum(avg_2)) / len(avg_2)) if len(avg_2) else -1]

...
def winner(deck_steve, deck_josh):
    ...
    for i in range(len(deck_steve)):
        if deck_steve[i] == deck_josh[i]:
            continue
        if cards.index(deck_steve[i]) > cards.index(deck_josh[i]):
            steve += 1
        else:
            josh += 1
    return "Tie" if josh == steve else f"{'Steve' if steve > josh else 'Josh'} wins {steve if steve > josh else josh} to {josh if josh < steve else steve}"

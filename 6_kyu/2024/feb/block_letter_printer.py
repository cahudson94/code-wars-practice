...
and returns that string in block letters of 5 characters width and 7 characters height, with one space between characters.

The string should be formatted in a way that when passed to Pythons' print() function shows the desired output (see below for example).

There's a preloaded dictionary called alpha which you can use. Keys are lower case letters and the space.
...
After completing this kata you might want to check out the [code golf] version as well.
"""

def block_print(string):
    string = string.strip()
    return "\n".join(" ".join(alpha[x][i] for x in string.lower()).rstrip() for i in range(7)) if string else ""

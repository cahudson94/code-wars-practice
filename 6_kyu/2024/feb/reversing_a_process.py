...
def decode(r):
    num = int("".join(x for x in r if x.isdigit()))
    char_map = {idx * num % 26: char for idx, char in enumerate("abcdefghijklmnopqrstuvwxyz")}
    if len(char_map) < 26:
        return "Impossible to decode"
    return "".join(char_map[ord(x) - 97] for x in r if not x.isdigit())

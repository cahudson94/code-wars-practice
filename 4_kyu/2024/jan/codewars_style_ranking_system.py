...
class User:
    rank = -8
    progress = 0
    progress_change = {-1: 1, 0: 3}
    for i in range(1, 16):
        progress_change[i] = 10 * i ** 2
        
    def inc_progress(self, challenge):
        if not challenge or -8 > challenge or 8 < challenge:
            raise ValueError("Not a valid difficulty")
        diff = challenge - (challenge > 0) - self.rank + (self.rank > 0)
        if diff < -1 or self.rank == 8:
            return
        total_progress = self.progress + self.progress_change.get(diff)
        self.progress = total_progress % 100
        rank_ups = total_progress // 100
        while rank_ups and self.rank < 8:
            rank_ups -= 1

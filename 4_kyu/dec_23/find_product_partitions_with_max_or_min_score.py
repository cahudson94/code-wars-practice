...
Suposse that we have that n = 1416 The product partitions of this number with a corresponding special score are as follows:
...
The algorithm to produce the product partition is a key factor in terms of speed.
...
def find_spec_prod_part(n, com):
    ...
    for partition in partitions(n):
        if len(partition) < 2:
            continue
        score = sum(x ** partition.count(x) for x in set(partition)) * len(partition)
        if score_out == None or eval(com)(score, score_out) != score_out:
            partition_out = partition
            score_out = score
    
    return "It is a prime number" if not score_out else [sorted(partition_out, reverse=True), score_out]

...
Read PEP 636 may help you understand it.
...
(This is just part of it, read PEP635 will give you full understanding of it.)
    ...
    The if ... elif ... elif ... else idiom is often used to find out the type or shape of an object in an ad-hoc fashion,
    using one or more checks like isinstance(x, cls), hasattr(x, "attr"), len(x) == n or "key" in x as guards to select an applicable block.
    The block can then assume x supports the interface checked by the guard. For example:
...
Return the result with the following rules:
    ...
    If arg is a list with 2 or more elements, convert the first and last elements to integer if not yet integers,
    then return first element divided by the last element (float division).

...
one liner:
    return all([key[i] == answers[i] or key[i] == "_" for i in range(len(key))]) or all([key[i] != answers[i] or key[i] == "_" for i in range(len(key))])
...
def possibly_perfect(key, answers):
    ...
    for i in range(len(key)):
        if key[i] == "_":
            continue
        if mode is None:
            mode = key[i] == answers[i]
        if mode != (key[i] == answers[i]):
            perfect = False
    return perfect

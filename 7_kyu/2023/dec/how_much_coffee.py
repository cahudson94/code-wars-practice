...
def how_much_coffee(events):
    lower_types = ["cw", "dog", "cat", "movie"]
    upper_types = [x.upper() for x in lower_types]
    coffees = sum([1 if x in lower_types else 2 if x in upper_types else 0 for x in events])
    return coffees if coffees <= 3 else "You need extra sleep"

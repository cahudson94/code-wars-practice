...
def max_sum_dig(n_max, max_sum):
    good_nums = []
    for x in range(1000, n_max + 1):
        sum_is_good = True
        for digits in [str(x)[idx:idx+4] for idx, _ in enumerate(str(x)[:-3])]:
            if sum([int(x) for x in digits]) > max_sum:
                sum_is_good = False
        if sum_is_good:
            good_nums.append(x)
    total, count = sum(good_nums), len(good_nums)
    nearest_mean = min(good_nums, key = lambda x: abs(total - x * count))
	return [count, nearest_mean, total]

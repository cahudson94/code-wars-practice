...
The 'water pouring' problem (a.k.a 'two water jug' problem, or the even cooler 'Die Hard with a vengeance' puzzle)
is a puzzle involving a finite number of water jugs with integer capacities (in this case, measured in litres).
For this problem, we are using only two jugs. Initially, each jug is empty.
We want to find a series of 'steps' to reach a state where one of the two jugs contain the desired quantity of water.
Task
Inputs:
...
In short, a valid step consists of pouring water from a source container (either of the two jugs or from the infinite source of water)
to a destination (either of the two jugs or poured outside) until either the source container is empty or the destination container is full.
...
def wpp(a, b, n):
    ...
    for i in range(b):
        ...
        d = min(c + a, b)
        steps.append((a, c))
        steps.append((c + a - d, d))
        if n in (c + a - d, d):
        	return steps
        if c + a >= b:
            steps.append((c + a - d,0))
            steps.append((0, c + a - d))
    return []

...
def code(st):
    if not st:
        return ""
    nearest_square = ceil(len(st) ** 0.5)
    if len(st) ** 0.5 != nearest_square:
        st += chr(11) * (nearest_square ** 2 - len(st))
    out_lines = [[x for x in st[i * nearest_square:(i + 1) * nearest_square]] for i in range(len(st) // nearest_square)]
    return "\n".join(["".join(x for x in line) for line in zip(*out_lines[::-1])])
...
def decode(st):
    if not st:
        return st
    lines = list(zip(*[[x for x in line] for line in st.split("\n")]))[::-1]
    return "".join("".join(x for x in line) for line in lines).rstrip(chr(11))

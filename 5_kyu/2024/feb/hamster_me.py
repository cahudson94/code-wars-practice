...
They have become established as popular small house pets, and,
partly because they are easy to breed in captivity, hamsters are often used as laboratory animals.
...
All letters which directly follow letters from code get number 2 (unless they already have a smaller number assigned), etc.
...
def hamster_me(code, message):
    letters = {x: f"{x}1" for x in code}
    alpha = "abcdefghijklmnopqrstuvwxyz"
    ordered_code = sorted(code)
    for idx, x in enumerate(ordered_code):
        child_letters = alpha[ord(x) - 96:] + alpha[:ord(x) - 97]
        offset = 2
        for y in child_letters:
            if y not in letters:
                letters[y] = f"{x}{offset}"
                offset += 1
            else:
                break
    return "".join(letters[x] for x in message)

...
If you did my Resistor Color Codes kata, you wrote a function which took a string containing a resistor's band colors,
and returned a string identifying the resistor's ohms and tolerance values.

Well, now you need that in reverse:
The previous owner of your "Beyond-Ultimate Raspberry Pi Starter Kit" (as featured in my Fizz Buzz Cuckoo Clock kata)
had emptied all the tiny labeled zip-lock bags of components into the box, so that for each resistor you need for a project,
instead of looking for text on a label, you need to find one with the sequence of band colors that matches the ohms value you need.
...
followed by a space and the word "ohms" (to avoid Codewars unicode headaches I'm just using the word instead of the ohms unicode symbol).
The way an ohms value is formatted depends on the magnitude of the value:
    ...
    For example, with the 47-ohm resistor above, your function would receive the string "47 ohms", and return the string `"yellow violet black gold".
...
def encode_resistor_colors(ohms_string):
    ...
    if "." in ohms_string:
        ohms = ohms_string.replace(".", "").replace("M", "00000").replace("k", "00").split()[0]
    else:
        ohms = ohms_string.replace("M", "000000").replace("k", "000").split()[0]        
    band_1 = bands.get(int(ohms[0]))
    band_2 = bands.get(int(ohms[1]))
    ohm_power = 0
    while 10 ** ohm_power * int(ohms[:2]) != int(ohms):
        ohm_power += 1
    band_3 = bands.get(ohm_power)
    return " ".join([band_1, band_2, band_3, "gold"])

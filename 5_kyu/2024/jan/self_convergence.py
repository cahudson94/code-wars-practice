...
def self_converge(n, iters=1, seen=[]):
    if iters == 1: # code wars was persisting seen across tests
        seen = []
    n = str(n)
    des, asc = int(''.join(sorted(n))), int(''.join(sorted(n, reverse=True)))
    next_n = str(max(des, asc) - min(des, asc))
    n = "0" * (len(n) - len(next_n)) + next_n
    if n in seen:
        return iters
    seen.append(n)
    iters += 1
    return self_converge(n, iters, seen)

...
def get_squares(x):
    squares = set()
    for combo in permutations(str(x), len(str(x))):
        if int(''.join(combo)) ** 0.5 == int(int(''.join(combo)) ** 0.5):
            squares.add(int(''.join(combo)))
    return len(squares)

def sort_by_perfsq(arr):
    return sorted(sorted(arr), key=get_squares, reverse=True)

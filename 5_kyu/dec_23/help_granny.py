...
def get_leg_distance(a, c):
    return (c ** 2 - a ** 2) ** 0.5

def tour(friends, friend_towns, home_to_town_distances):
    friend_town_map = {town[0]: town[1] for town in friend_towns}
    last_town = friend_town_map[friends[0]]
    total_dist = home_to_town_distances[last_town]
    for friend in friends:
        try:
            total_dist += get_leg_distance(home_to_town_distances[last_town], home_to_town_distances[friend_town_map[friend]])
            last_town = friend_town_map[friend]
        except KeyError:
            pass
    return int(total_dist + home_to_town_distances[last_town])
